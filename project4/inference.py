...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        for i in range(self.numParticles):
            self.particles.append(tuple([random.choice(self.legalPositions) for _ in range(self.numGhosts)]))
        # util.raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            newParticle = []
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, particle),
                                                       i, self.ghostAgents[i])
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # Setting tags for Ghost Capture Status
        ...
        def captureGhost():
            ...
            if None not in noisyDistances:
                ...
                tmp_particles = []
                for i, each in enumerate(noisyDistances):
                    if each is None:
                        #captured[i] = True
                        for particle in self.particles:
                            tmp_particle = list(particle)
                            tmp_particle[i] = self.getJailPosition(i)
                            tmp_particles.append(tuple(tmp_particle))
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if particle[i] != self.getJailPosition(i):
                    ...
                    likelihood *= emissionModels[i][true_distance]
            dist[particle] += likelihood

        #resample particles based on the likelihood
        if dist.totalCount() == 0:
            ...
            newParticles = []
            for particle in self.particles:
                newParticle = tuple(util.sample(dist))
                newParticles.append(newParticle)
            self.particles = newParticles
