...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    # First computes the most likely position of each ghost
    # ghost.argMax() return the most likely position
    mostLikelyGhosts = []
    ...
    for position in mostLikelyGhosts:
      ...
      if distance<closestGhostDis:
        maxProbPosition = position
        closestGhostDis = distance
    ...
    for action in legal:
      ...
      distance = self.distancer.getDistance(maxProbPosition, successorPosition)
